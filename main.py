@namespace
class SpriteKind:
    atk = SpriteKind.create()

def on_life_zero():
    game.show_long_text("Level:" + str(lvl), DialogLayout.BOTTOM)
...
sprites.on_overlap(SpriteKind.atk, SpriteKind.enemy, on_on_overlap)

def on_on_overlap2(sprite2, otherSprite2):
    info.change_life_by(15)
...
def on_update_interval():
    global ghost, Slash, slash2, poulet
    ...
    for index in range(lvl):
        if randint(1, 4) == 1:
            ghost.follow(Hero, 70)
            ghost.set_position(0, randint(0, 120))
        elif randint(1, 4) == 2:
            ghost.follow(Hero, 70)
            ghost.set_position(160, randint(0, 120))
        elif randint(1, 4) == 3:
            ghost.follow(Hero, 70)
            ghost.set_position(randint(0, 160), 0)
        else:
            ghost.follow(Hero, 70)
            ghost.set_position(randint(0, 160), 120)
    ...
    Slash.lifespan = 300
    slash2.lifespan = 300
    Slash.set_position(Hero.x + 10, Hero.y)
    slash2.set_position(Hero.x - 20, Hero.y)
    if randint(0, 9) == 0:
        poulet = sprites.create(img("""
                . . 2 2 b b b b b . . . . . . . 
                            . 2 b 4 4 4 4 4 4 b . . . . . . 
            ...
            """),
            SpriteKind.food)
        poulet.set_position(randint(0, 160), randint(0, 120))
game.on_update_interval(1000, on_update_interval)

def on_update_interval2():
    global lvl
    if lvl * 10 < info.score():
        lvl += 1
    if ghost.vx < 0:
        ghost.set_image(img("""
            ........................
